...
public final class Target
	{
		...
		public int getShootsLeft()
			{
				return (hitsToKillTarget - hitsSoFar);
			}
		...
		public void draw(Graphics2D g)
			{
				Integer shootsLeft = new Integer(0);
				...
				if (isDead() == false)
					{
						g.fillArc(drawP.x - (int) radius, drawP.y - (int) radius, (int) getDiameter(),
								(int) getDiameter(), 0, 360);
						g.setColor(Color.black);

						shootsLeft = (hitsToKillTarget - hitsSoFar);
						shootLeftStr = shootsLeft.toString();

						g.drawString(shootLeftStr, drawP.x - 3, drawP.y + 3);
					}
				if (isDead() == true)
					{
						g.drawArc(drawP.x - (int) radius, drawP.y - (int) radius, (int) getDiameter(),
								(int) getDiameter(), 0, 360);
					}
		...
		public void closeToBorder(boolean b)
			{
				if (b)
					{
						if (hitsSoFar < hitsToKillTarget)
